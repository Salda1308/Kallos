...
- **If `kallos/` does not exist:** create `kallos/referencias/` and `kallos/output/` (empty), and copy `~/.claude/skills/kallos-contentGen/templates/brief.template.md` to `kallos/brief.md`. Report that it was created and that the user needs to fill in `brief.md` and drop reference files (images/PDFs) into `referencias/` before asking for a design.
